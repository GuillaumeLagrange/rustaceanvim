...
---Get the settings by from config and vscode settings if found.
---settings and applies them.
---@param bufname string
...
M.reload_settings = function(bufnr)
  vim.print('Hello from reload_settings')
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  local clients = rust_analyzer.get_active_rustaceanvim_clients(bufnr)
  for _, client in ipairs(clients) do
...
vim.api.nvim_create_user_command('RustAnalyzer', rust_analyzer_cmd, {
  ...
  complete = function(arg_lead, cmdline, _)
    ...
    local commands = #clients == 0 and { 'start' } or { 'stop', 'restart', 'reload_settings' }
